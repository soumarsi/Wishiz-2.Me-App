...
-(NSArray *)getPermissionsArray
{
    ...
    */
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        permissions = [[NSArray alloc] initWithObjects:@"user_birthday",@"email",@"public_profile",@"user_location",@"user_hometown", nil];
    });
    return permissions;
...
{
    FBRequest* friendsRequest = [[FBRequest alloc]initWithSession:self.session graphPath:[NSString stringWithFormat:@"%@/friends?fields=picture.width(800).height(800),name&limit=%d",userID,limit]];
...
{
    FBRequest* friendsRequest = [[FBRequest alloc]initWithSession:self.session graphPath:[NSString stringWithFormat:@"%@/likes?fields=picture.width(800).height(800),name&limit=%d",userID,limit]];
    [friendsRequest startWithCompletionHandler: ^(FBRequestConnection *connection,
                                                  NSDictionary* result,
                                                  NSError *error)
     {
