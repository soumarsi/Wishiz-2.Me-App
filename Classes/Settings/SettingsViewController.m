...
- (void)viewDidLoad
{
    NSMutableDictionary *dict = [[NSMutableDictionary alloc] initWithDictionary:[[NSUserDefaults standardUserDefaults] objectForKey:UD_FB_USER_DETAIL]];
    ...
    btnMale.selected = ([dict[@"gender"] isEqualToString:@"male"]);
    btnFemale.selected = !btnMale.selected;
}

-(void)upDatePrefControls{
    /***** settings For gender******/
    if ([UserSettings currentSetting].sex==nil) {
        [UserSettings currentSetting].sex=@"2";
    }
    
    if ([[UserSettings currentSetting].sex intValue]==2) {
    ...
    if ([[UserSettings currentSetting].prSex intValue]==2) {
        [switchMale setOn:YES animated:YES];
        [swichFemale setOn:NO animated:YES];
    }
    else if ([[UserSettings currentSetting].prSex intValue]==1) {
    ...
    if ([UserSettings currentSetting].prRad==nil) {
        [UserSettings currentSetting].prRad=@"100";
    }
...
-(void)saveUpdatedValue
{
    ...
    [sliderDistance setValue:100 animated:YES];
    sliderDistance.maximumValue =100;
    sliderDistance.minimumValue = 0;
    ...
    if (![ud objectForKey:@"DISTANCE"])
    {
        if ([ud integerForKey:@"DIST"] ==3) {
            lblDistance.text = [NSString stringWithFormat:@"%d Mi", 100];
        }
        else if ([ud integerForKey:@"DIST"] ==4) {
            lblDistance.text = [NSString stringWithFormat:@"%d Km", 100];
        }
        else{
            lblDistance.text =@"100 Mi";
        }
...
- (void)openMail
{
    [super sendMailSubject:@"Wishiz.me App!" toRecipents:[NSArray arrayWithObject:@"support@wishiz.me"] withMessage:@""];
}
...
{
    [[Helper sharedInstance] setIntCount:0];
    ProgressIndicator * pi = [ProgressIndicator sharedInstance];
    ...
    [afn getDataFromPath:METHOD_UPDATEPREFERENCES withParamData:dictParam withBlock:^(id response, NSError *error) {
        ...
        if (response) {
            if ([[response objectForKey:@"errFlag"] intValue]==0) {
                [[TinderAppDelegate sharedAppDelegate]showToastMessage:@"Settings updating successfully."];
            }else{
