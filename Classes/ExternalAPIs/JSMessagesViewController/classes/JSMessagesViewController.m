...
- (void)setup
{
    ...
    if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"fromWhere"] isEqualToString:@"otherProfileChat"]) {
...
//    }
    ...
    DebugLog(@"fundingCampaignId-----%@",[[NSUserDefaults standardUserDefaults] objectForKey:@"fundingCampaignId"]);
    DebugLog(@"CUrrentUser-----------%@",[[NSUserDefaults standardUserDefaults] objectForKey:@"currentUser"]);
    
    UILabel *proLabel = [[UILabel alloc]init];
    proLabel.frame = CGRectMake(0.0f, 20.0f, self.view.bounds.size.width, 70.0f);
    proLabel.backgroundColor = [UIColor colorWithRed:(145.0f/255.0f) green:(247.0f/255.0f) blue:(156.0f/255.0f) alpha:1.0f];
    [self.view addSubview:proLabel];
    
    UIImageView *itemPic = [[UIImageView alloc]init];
    itemPic.frame = CGRectMake(proLabel.frame.origin.x+5.0f, proLabel.frame.origin.y+5.0f, 60.0f, 50.0f);
    itemPic.backgroundColor = [UIColor clearColor];
    NSString *imageURL = [NSString stringWithFormat:@"%@%@%@%@",USER_FILES_URL,@"fundingCampaign_icon_", [[NSUserDefaults standardUserDefaults] objectForKey:@"fundingCampaignId"], @".jpg"];
    [itemPic sd_setImageWithURL:[NSURL URLWithString:imageURL]
                      placeholderImage:[UIImage imageNamed:@"placeholder.png"]];
    [self.view addSubview:itemPic];
    
    UILabel *ItemLabel = [[UILabel alloc]init];
    ItemLabel.frame = CGRectMake(itemPic.frame.origin.x+itemPic.frame.size.width+8.0f, itemPic.frame.origin.y, proLabel.frame.size.width-80.0f, 50.0f);
    ItemLabel.backgroundColor = [UIColor clearColor];
    ItemLabel.textColor = [UIColor blackColor];
    ItemLabel.font = [UIFont systemFontOfSize:13.0f];
    ItemLabel.numberOfLines = 3;
    ItemLabel.text =[NSString stringWithFormat:@"%@ wants to learn more about your %@ Wish!",[[NSUserDefaults standardUserDefaults]objectForKey:@"currentUser"],[[NSUserDefaults standardUserDefaults] objectForKey:@"itemName"]];
    ...
    if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"fromWhere"] isEqualToString:@"otherProfileChat"]) {
        ...
        tableFrame = CGRectMake(0.0f, 80.0f, size.width, size.height - inputViewHeight-80.0f); //--------------PK
    
    }else{
        
        tableFrame = CGRectMake(0.0f, 0.0f, size.width, size.height - inputViewHeight); //--------------PK
    }
