...
{
    [self pushViewController:controller onDirection:direction withOffset:DefaultOffset animated:animated forceToPopPush:forcePopPush];
}

- (void) pushOldViewControllerOnDirection:(PPRevealSideDirection)direction animated:(BOOL)animated
{
    [self pushOldViewControllerOnDirection:direction withOffset:DefaultOffset animated:animated];
}
...
- (void) pushOldViewControllerOnDirection:(PPRevealSideDirection)direction withOffset:(CGFloat)offset animated:(BOOL)animated
{
    ...
    if (oldController)
    {
        [self pushViewController:oldController onDirection:direction withOffset:offset animated:animated];
    }
    else
    {
    ...
    if (interactions & PPRevealSideInteractionContentView) {
        ...
        if ([controller isKindOfClass:[UINavigationController class]]) {
            ...
            if (controller.navigationController)
                ...
                c = controller;
        if ([c isKindOfClass:[JSDemoViewController class]]) {
            //do nothing
        }else
        {
            if ([c isKindOfClass:[SettingsViewController class]])
            {
                
                SettingsViewController *settings =  (SettingsViewController*)c;
                
                [settings saveUpdatedValue];
...
- (void) gestureRecognizerDidPan:(UIPanGestureRecognizer*)panGesture {
    ...
    /*
    if ([c isKindOfClass:[ChatViewController class]]) {
        ChatViewController *chat =  (ChatViewController*)c;
    }
    */
    if (c) {
        if (!c.view.superview)
...
//            if (_currentPanDirection == PPRevealSideDirectionRight) {
//                [_rootViewController viewDidAppear:YES];
//            }
